...
const RSVPModalContent: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const modalClasses = "bg-[#1a0f0a] border border-white/10 rounded-[3rem] w-full max-w-lg p-8 sm:p-12 relative shadow-2xl overflow-hidden";
  const contentInitial = { opacity: 0, scale: 0.95 };
      ...
      {isSuccess ? (
            ...
            <div className="flex gap-4">
              ...
              <motion.button
                onClick={() => {
                  onClose();
                  setTimeout(() => {
                    setStep(1); setGuestName(""); setStarterChoice("none"); setMealChoice("none"); setComment(""); setExistingResponseId(null); setShowUpdatePrompt(false);
                  }, 500);
                }}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
        ...
        <motion.div key="step-2" layoutId="rsvp-container" initial={contentInitial} animate={contentAnimate} exit={contentExit} transition={ELASTIC_SPRING} className={modalClasses}>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={innerTransition}>
            ...
            <div className="space-y-4 mb-8">
              <motion.button onClick={() => setStarterChoice("jambon")} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={ELASTIC_SPRING} className={"w-full p-6 rounded-2xl border transition-all flex items-center justify-between " + (starterChoice === "jambon" ? "bg-rose-500/20 border-rose-500/50 text-rose-50" : "bg-white/5 border-white/10 text-rose-200/60")}>
              ...
              <motion.button onClick={() => setStarterChoice("tomate")} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={ELASTIC_SPRING} className={"w-full p-6 rounded-2xl border transition-all flex items-center justify-between " + (starterChoice === "tomate" ? "bg-rose-500/20 border-rose-500/50 text-rose-50" : "bg-white/5 border-white/10 text-rose-200/60")}>
              ...
              <motion.button onClick={() => setStarterChoice("crevette")} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={ELASTIC_SPRING} className={"w-full p-6 rounded-2xl border transition-all flex items-center justify-between " + (starterChoice === "crevette" ? "bg-rose-500/20 border-rose-500/50 text-rose-50" : "bg-white/5 border-white/10 text-rose-200/60")}>
        ...
        <motion.div key="step-3" layoutId="rsvp-container" initial={contentInitial} animate={contentAnimate} exit={contentExit} transition={ELASTIC_SPRING} className={modalClasses}>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={innerTransition}>
            ...
            <div className="space-y-4 mb-8">
              <motion.button onClick={() => setMealChoice("perche")} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={ELASTIC_SPRING} className={"w-full p-6 rounded-2xl border transition-all flex items-center justify-between " + (mealChoice === "perche" ? "bg-rose-500/20 border-rose-500/50 text-rose-50" : "bg-white/5 border-white/10 text-rose-200/60")}>
                <span className="text-left"><span className="block font-medium text-lg">Filets de perche</span><span className="text-sm opacity-60 block">Frites</span></span>
                {mealChoice === "perche" && <Check className="w-6 h-6 text-rose-300" />}
              </motion.button>
              <motion.button onClick={() => setMealChoice("boeuf")} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={ELASTIC_SPRING} className={"w-full p-6 rounded-2xl border transition-all flex items-center justify-between " + (mealChoice === "boeuf" ? "bg-rose-500/20 border-rose-500/50 text-rose-50" : "bg-white/5 border-white/10 text-rose-200/60")}>
                <span className="text-left"><span className="block font-medium text-lg">Médaillons de bœuf</span><span className="text-sm opacity-60 block">Sauce morilles, frites, légumes</span></span>
        ...
        <motion.div key="step-4" layoutId="rsvp-container" initial={contentInitial} animate={contentAnimate} exit={contentExit} transition={ELASTIC_SPRING} className={modalClasses}>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={innerTransition}>
            ...
            <motion.button disabled={isSubmitting} onClick={() => { setComment(""); handleSubmit(); }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={ELASTIC_SPRING} className="w-full mb-8 text-rose-200/40 hover:text-rose-200 text-sm italic transition-colors block text-center underline underline-offset-4 decoration-rose-500/30">
